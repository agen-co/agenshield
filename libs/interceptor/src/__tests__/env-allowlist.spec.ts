import { filterEnvByAllowlist, BASE_ENV_ALLOWLIST } from '@agenshield/seatbelt';
...
describe('filterEnvByAllowlist', () => {
  ...
  it('passes NODE_OPTIONS through allowlist (stripped later via envDeny)', () => {
    const result = filterEnvByAllowlist(fakeEnv);
    expect(result['NODE_OPTIONS']).toBe('--require /tmp/evil.js');
  });
