 ...
 *
 * When the process runs as root (via sudo, LaunchDaemon, or MDM),
 * os.homedir() returns /var/root. This module resolves the actual human
 * user's home directory so that keychain operations target the correct
 * keychain — the host user's, not root's.
 *
...
/**
 * Clear cached host user (for testing).
 */
export function clearHostUserCache(): void {
  cached = undefined;
}
