...
const stepExecutors: Record<WizardStepId, StepExecutor> = {
  ...
  'create-agent-user': async (context) => {
    ...
    try {
      const { addUserState } = await import('../state/index.js');
      addUserState({
        username: agentUser.username,
        uid: agentUser.uid,
        type: 'agent',
...
SHIELD_EOF`, { encoding: 'utf-8', stdio: 'pipe' });
  ...
  'setup-passcode': async (context) => {
    ...
    try {
      ...
      const { getVault } = await import('../vault/index.js');
      const vault = getVault();
      await vault.set('passcode', passcodeData);

      // Enable passcode protection in state
      const { updatePasscodeProtectionState } = await import('../state/index.js');
...
export function createWizardEngine(options?: WizardOptions): WizardEngine {
  ...
  const engine: WizardEngine = {
    ...
    async runSetupPhase() {
      ...
      const executor = context.privilegeExecutor;
      let hasTempSudoers = false;

      // If privilege executor is available (daemon/osascript mode), use it to
      // create a temporary sudoers rule granting the current user passwordless
      // sudo. This allows ALL existing sudo calls (in step executors and sandbox
      // library functions) to work without individual refactoring.
      if (executor && !context.options?.dryRun) {
        const currentUser = os.userInfo().username;
        logVerbose('Requesting administrator access for setup...', context);
        const result = await executor.execAsRoot(
          `sh -c 'printf "%s ALL=(ALL) NOPASSWD: ALL\\n" "${currentUser}" > /etc/sudoers.d/agenshield-setup-temp && chmod 440 /etc/sudoers.d/agenshield-setup-temp'`
        );
        if (!result.success) {
          throw new Error(`Failed to set up privileged access: ${result.error}`);
        }
        hasTempSudoers = true;
        logVerbose('Administrator access granted', context);
      }

      // Fallback: when no executor (CLI/terminal mode), keep sudo cache warm
      const keepalive = !executor && !context.options?.dryRun
        ? setInterval(() => {
      ...
      try {
        ...
        if (hasTempSudoers && executor) {
          try {
            await executor.execAsRoot('rm -f /etc/sudoers.d/agenshield-setup-temp');
            logVerbose('Temporary privileged access revoked', context);
