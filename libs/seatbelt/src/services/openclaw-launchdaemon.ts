 ...
 */
export async function generateOpenClawDaemonPlist(config: OpenClawLaunchConfig): Promise<string> {
  const socketPath = config.socketPath || ipcSocketPath(config.agentHome);
  const interceptorPath = config.interceptorPath || '/opt/agenshield/lib/interceptor/register.cjs';
  const httpPort = config.httpPort || 5201;
  const logsDir = logDir(config.agentHome);

  const hostAppExists = await fs.access('/Applications/AgenShield.app').then(() => true, () => false);
 ...
 */
export async function generateOpenClawGatewayPlist(config: OpenClawLaunchConfig): Promise<string> {
  const socketPath = config.socketPath || ipcSocketPath(config.agentHome);
  const interceptorPath = config.interceptorPath || '/opt/agenshield/lib/interceptor/register.cjs';
  const httpPort = config.httpPort || 5201;
  const logsDir = logDir(config.agentHome);
...
export async function installOpenClawLaunchDaemons(
  ...
  try {
    ...
    const gatewayPlist = await generateOpenClawGatewayPlist(config);
