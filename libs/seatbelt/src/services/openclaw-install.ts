...
async function sudoExec(cmd: string): Promise<{ success: boolean; output?: string; error?: string }> {
  try {
...
/**
 * Async version of detectHostOpenClawVersion.
 */
export async function detectHostOpenClawVersionAsync(): Promise<string | null> {
  try {
 ...
 */
export async function copyOpenClawConfig(options: {
  ...
  verbose?: boolean;
  onLog?: (msg: string) => void;
}): Promise<OpenClawConfigCopyResult> {
  ...
  try {
    const sourceExists = await fsp.access(sourceConfigPath).then(() => true, () => false);
    if (!sourceExists) {
      log('Source config path does not exist, creating empty .openclaw');
      await sudoExec(`mkdir -p "${path.join(targetConfigDir, 'workspace', 'skills')}" "${path.join(targetConfigDir, 'canvas')}"`);
      await sudoExec(`chown -R ${agentUsername}:${socketGroup} "${targetConfigDir}"`);
      await sudoExec(`chmod 2775 "${targetConfigDir}"`);
      return {
        success: true,
    ...
    const targetExists = await fsp.access(targetConfigDir).then(() => true, () => false);
    if (targetExists) {
      await sudoExec(`rm -rf "${targetConfigDir}"`);
    }
    ...
    const cpResult = await sudoExec(`cp -a "${sourceConfigPath}" "${targetConfigDir}"`);
    if (!cpResult.success) {
      return {
        success: false,
        configDir: targetConfigDir,
    ...
    if (originalHome !== agentHome) {
      ...
      const sedResult = await sudoExec(
      ...
      try {
        const { stdout: rawJson } = await execAsync(`sudo cat "${configJsonPath}"`);
        const parsed = JSON.parse(rawJson) as Record<string, unknown>;
        const rewritten = JSON.stringify(parsed, null, 2).replaceAll(originalHome, agentHome);
        await execAsync(`sudo tee "${configJsonPath}" > /dev/null`, { env: { ...process.env }, ...({ input: rewritten } as any) });
    ...
    for (const subdir of ['canvas', 'workspace', 'workspace/skills', 'logs']) {
      ...
      await sudoExec(`mkdir -p "${dirPath}"`);
      await sudoExec(`chown ${agentUsername}:${socketGroup} "${dirPath}"`);
      await sudoExec(`chmod 2775 "${dirPath}"`);
    }
    ...
    const configJsonExists = await fsp.access(configJson).then(() => true, () => false);
    if (configJsonExists) {
      await sudoExec(`chmod 664 "${configJson}"`);
    }
 ...
 */
async function findOpenClawProcesses(username: string): Promise<{ daemon: number[]; gateway: number[]; other: number[] }> {
  const result: { daemon: number[]; gateway: number[]; other: number[] } = { daemon: [], gateway: [], other: [] };

  try {
    // Find all openclaw processes owned by the user
    const psResult = await sudoExec(`ps -u ${username} -o pid,command`);
    if (!psResult.output) return result;

    for (const line of psResult.output.split('\n')) {
...
export async function stopHostOpenClaw(options: {
  ...
  const procs = await findOpenClawProcesses(originalUser);
  ...
  const remaining = await findOpenClawProcesses(originalUser);
  ...
  if (allPids.length > 0) {
    ...
    const stubborn = await findOpenClawProcesses(originalUser);
 ...
 */
export async function getHostOpenClawConfigPath(username?: string): Promise<string | null> {
  const user = username || getOriginalUser();
  const configPath = `/Users/${user}/.openclaw`;
  const exists = await fsp.access(configPath).then(() => true, () => false);
  if (exists) {
    return configPath;
  }
  return null;
...
export async function startAgentOpenClawDashboard(options: {
  ...
  try {
    // Ensure log directory and files exist (user-writable, no sudo needed)
    await fsp.mkdir(dashLogDir, { recursive: true });
