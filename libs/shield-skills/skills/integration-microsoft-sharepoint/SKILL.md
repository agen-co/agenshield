...
```bash
agentlink search-tools '{"queries":["microsoft sharepoint list sites"]}'
```
...
agentlink call-tool '{"toolName":"microsoft-sharepoint_list_sites","input":{...}}'
```

> If Microsoft SharePoint is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
