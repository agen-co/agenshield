...
```bash
agentlink search-tools '{"queries":["gmail list messages"]}'
```
...
agentlink call-tool '{"toolName":"gmail_list_messages","input":{...}}'
```

> If Gmail is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
