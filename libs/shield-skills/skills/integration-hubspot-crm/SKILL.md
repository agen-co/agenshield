...
```bash
agentlink search-tools '{"queries":["hubspot crm list contacts"]}'
```
...
agentlink call-tool '{"toolName":"hubspot-crm_list_contacts","input":{...}}'
```

> If HubSpot CRM is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
