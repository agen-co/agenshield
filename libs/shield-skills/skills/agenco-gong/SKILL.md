...
```bash
agenco search-tools '{"queries":["gong list calls"]}'
```
...
agenco call-tool '{"toolName":"gong_list_calls","input":{...}}'
```

> If Gong is not connected, run `agenco list-connected-integrations` to check, then connect it via the Shield UI.
