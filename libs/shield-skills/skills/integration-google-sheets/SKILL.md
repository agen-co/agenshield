...
```bash
agentlink search-tools '{"queries":["google sheets get spreadsheet"]}'
```
...
agentlink call-tool '{"toolName":"google-sheets_get_spreadsheet","input":{...}}'
```

> If Google Sheets is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
