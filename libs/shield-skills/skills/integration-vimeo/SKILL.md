...
```bash
agentlink search-tools '{"queries":["vimeo list videos"]}'
```
...
agentlink call-tool '{"toolName":"vimeo_list_videos","input":{...}}'
```

> If Vimeo is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
