...
```bash
agentlink search-tools '{"queries":["figma get file"]}'
```
...
agentlink call-tool '{"toolName":"figma_get_file","input":{...}}'
```

> If Figma is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
