...
```bash
agenco search-tools '{"queries":["segment list sources"]}'
```
...
agenco call-tool '{"toolName":"segment_list_sources","input":{...}}'
```

> If Segment is not connected, run `agenco list-connected-integrations` to check, then connect it via the Shield UI.
