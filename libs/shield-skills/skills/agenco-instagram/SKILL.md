...
```bash
agenco search-tools '{"queries":["instagram get profile"]}'
```
...
agenco call-tool '{"toolName":"instagram_get_profile","input":{...}}'
```

> If Instagram is not connected, run `agenco list-connected-integrations` to check, then connect it via the Shield UI.
