...
```bash
agentlink search-tools '{"queries":["akamai list properties"]}'
```
...
agentlink call-tool '{"toolName":"akamai_list_properties","input":{...}}'
```

> If Akamai is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
