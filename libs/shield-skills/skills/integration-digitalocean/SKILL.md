...
```bash
agentlink search-tools '{"queries":["digitalocean get account"]}'
```
...
agentlink call-tool '{"toolName":"digitalocean_get_account","input":{...}}'
```

> If DigitalOcean is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
