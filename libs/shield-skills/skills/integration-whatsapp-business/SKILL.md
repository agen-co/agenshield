---
name: integration-whatsapp-business
...
```bash
agentlink search-tools '{"queries":["whatsapp business send text message"]}'
```
...
agentlink call-tool '{"toolName":"whatsapp-business_send_text_message","input":{...}}'
```

> If WhatsApp Business is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
