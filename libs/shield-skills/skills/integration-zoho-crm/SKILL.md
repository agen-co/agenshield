...
```bash
agentlink search-tools '{"queries":["zoho crm list leads"]}'
```
...
agentlink call-tool '{"toolName":"zoho-crm_list_leads","input":{...}}'
```

> If Zoho CRM is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
