...
```bash
agentlink search-tools '{"queries":["drift list conversations"]}'
```
...
agentlink call-tool '{"toolName":"drift_list_conversations","input":{...}}'
```

> If Drift is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
