...
```bash
agentlink search-tools '{"queries":["microsoft onedrive list files"]}'
```
...
agentlink call-tool '{"toolName":"microsoft-onedrive_list_files","input":{...}}'
```

> If Microsoft OneDrive is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
