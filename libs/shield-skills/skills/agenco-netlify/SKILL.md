...
```bash
agenco search-tools '{"queries":["netlify list sites"]}'
```
...
agenco call-tool '{"toolName":"netlify_list_sites","input":{...}}'
```

> If Netlify is not connected, run `agenco list-connected-integrations` to check, then connect it via the Shield UI.
