...
```bash
agentlink search-tools '{"queries":["adp list workers"]}'
```
...
agentlink call-tool '{"toolName":"adp_list_workers","input":{...}}'
```

> If ADP is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
