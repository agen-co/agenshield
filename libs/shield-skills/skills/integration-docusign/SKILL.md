...
```bash
agentlink search-tools '{"queries":["docusign create envelope"]}'
```
...
agentlink call-tool '{"toolName":"docusign_create_envelope","input":{...}}'
```

> If DocuSign is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
