---
name: integration-constant-contact
...
```bash
agentlink search-tools '{"queries":["constant contact list contacts"]}'
```
...
agentlink call-tool '{"toolName":"constant-contact_list_contacts","input":{...}}'
```

> If Constant Contact is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
