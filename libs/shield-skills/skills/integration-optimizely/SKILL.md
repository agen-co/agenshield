...
```bash
agentlink search-tools '{"queries":["optimizely list projects"]}'
```
...
agentlink call-tool '{"toolName":"optimizely_list_projects","input":{...}}'
```

> If Optimizely is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
