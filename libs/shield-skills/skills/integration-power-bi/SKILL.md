...
```bash
agentlink search-tools '{"queries":["power bi list datasets"]}'
```
...
agentlink call-tool '{"toolName":"power-bi_list_datasets","input":{...}}'
```

> If Power BI is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
