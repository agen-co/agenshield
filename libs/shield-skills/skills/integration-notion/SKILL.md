...
```bash
agenco search-tools '{"queries":["notion get page"]}'
```
...
agenco call-tool '{"toolName":"notion_get_page","input":{...}}'
```

> If Notion is not connected, run `agenco list-connected-integrations` to check, then connect it via the Shield UI.
