...
```bash
agentlink search-tools '{"queries":["brevo list contacts"]}'
```
...
agentlink call-tool '{"toolName":"brevo_list_contacts","input":{...}}'
```

> If Brevo is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
