---
name: integration-microsoft-power-automate
...
```bash
agentlink search-tools '{"queries":["microsoft power automate list flows"]}'
```
...
agentlink call-tool '{"toolName":"microsoft-power-automate_list_flows","input":{...}}'
```

> If Microsoft Power Automate is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
