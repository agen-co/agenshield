...
```bash
agentlink search-tools '{"queries":["contentful list spaces"]}'
```
...
agentlink call-tool '{"toolName":"contentful_list_spaces","input":{...}}'
```

> If Contentful is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
