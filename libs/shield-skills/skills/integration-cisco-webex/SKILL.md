...
```bash
agentlink search-tools '{"queries":["cisco webex get current user"]}'
```
...
agentlink call-tool '{"toolName":"cisco-webex_get_current_user","input":{...}}'
```

> If Cisco Webex is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
