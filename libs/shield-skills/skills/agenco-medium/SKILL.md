...
```bash
agenco search-tools '{"queries":["medium get current user"]}'
```
...
agenco call-tool '{"toolName":"medium_get_current_user","input":{...}}'
```

> If Medium is not connected, run `agenco list-connected-integrations` to check, then connect it via the Shield UI.
