...
```bash
agentlink search-tools '{"queries":["launchdarkly list flags"]}'
```
...
agentlink call-tool '{"toolName":"launchdarkly_list_flags","input":{...}}'
```

> If LaunchDarkly is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
