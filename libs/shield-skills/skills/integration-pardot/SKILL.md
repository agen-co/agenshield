...
```bash
agentlink search-tools '{"queries":["pardot list prospects"]}'
```
...
agentlink call-tool '{"toolName":"pardot_list_prospects","input":{...}}'
```

> If Pardot is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
