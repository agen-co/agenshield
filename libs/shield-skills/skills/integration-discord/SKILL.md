...
```bash
agentlink search-tools '{"queries":["discord get current user"]}'
```
...
agentlink call-tool '{"toolName":"discord_get_current_user","input":{...}}'
```

> If Discord is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
