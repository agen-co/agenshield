...
```bash
agentlink search-tools '{"queries":["formstack list forms"]}'
```
...
agentlink call-tool '{"toolName":"formstack_list_forms","input":{...}}'
```

> If Formstack is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
