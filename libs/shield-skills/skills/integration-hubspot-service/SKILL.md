...
```bash
agentlink search-tools '{"queries":["hubspot service list tickets"]}'
```
...
agentlink call-tool '{"toolName":"hubspot-service_list_tickets","input":{...}}'
```

> If HubSpot Service is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
