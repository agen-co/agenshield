...
```bash
agentlink search-tools '{"queries":["grafana list dashboards"]}'
```
...
agentlink call-tool '{"toolName":"grafana_list_dashboards","input":{...}}'
```

> If Grafana is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
