...
```bash
agenco search-tools '{"queries":["square create payment"]}'
```
...
agenco call-tool '{"toolName":"square_create_payment","input":{...}}'
```

> If Square is not connected, run `agenco list-connected-integrations` to check, then connect it via the Shield UI.
