...
```bash
agentlink search-tools '{"queries":["webflow list sites"]}'
```
...
agentlink call-tool '{"toolName":"webflow_list_sites","input":{...}}'
```

> If Webflow is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
