...
```bash
agentlink search-tools '{"queries":["shopify list products"]}'
```
...
agentlink call-tool '{"toolName":"shopify_list_products","input":{...}}'
```

> If Shopify is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
