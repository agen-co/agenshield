---
name: agenco-calendly-enterprise
...
```bash
agenco search-tools '{"queries":["calendly enterprise get organization"]}'
```
...
agenco call-tool '{"toolName":"calendly-enterprise_get_organization","input":{...}}'
```

> If Calendly Enterprise is not connected, run `agenco list-connected-integrations` to check, then connect it via the Shield UI.
