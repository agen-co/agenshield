...
```bash
agenco search-tools '{"queries":["campaign monitor list clients"]}'
```
...
agenco call-tool '{"toolName":"campaign-monitor_list_clients","input":{...}}'
```

> If Campaign Monitor is not connected, run `agenco list-connected-integrations` to check, then connect it via the Shield UI.
