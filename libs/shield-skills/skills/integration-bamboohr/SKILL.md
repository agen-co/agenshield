...
```bash
agentlink search-tools '{"queries":["bamboohr list employees"]}'
```
...
agentlink call-tool '{"toolName":"bamboohr_list_employees","input":{...}}'
```

> If BambooHR is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
