...
```bash
agentlink search-tools '{"queries":["azure devops list projects"]}'
```
...
agentlink call-tool '{"toolName":"azure-devops_list_projects","input":{...}}'
```

> If Azure DevOps is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
