...
```bash
agentlink search-tools '{"queries":["salesforce query records"]}'
```
...
agentlink call-tool '{"toolName":"salesforce_query_records","input":{...}}'
```

> If Salesforce is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
