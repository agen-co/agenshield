...
```bash
agentlink search-tools '{"queries":["surveymonkey list surveys"]}'
```
...
agentlink call-tool '{"toolName":"surveymonkey_list_surveys","input":{...}}'
```

> If SurveyMonkey is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
