...
```bash
agentlink search-tools '{"queries":["activecampaign list contacts"]}'
```
...
agentlink call-tool '{"toolName":"activecampaign_list_contacts","input":{...}}'
```

> If ActiveCampaign is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
