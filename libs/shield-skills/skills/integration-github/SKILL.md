...
```bash
agentlink search-tools '{"queries":["github list repositories"]}'
```
...
agentlink call-tool '{"toolName":"github_list_repositories","input":{...}}'
```

> If GitHub is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
