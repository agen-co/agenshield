...
```bash
agentlink search-tools '{"queries":["asana get task"]}'
```
...
agentlink call-tool '{"toolName":"asana_get_task","input":{...}}'
```

> If Asana is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
