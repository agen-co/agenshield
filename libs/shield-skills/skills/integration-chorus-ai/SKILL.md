...
```bash
agentlink search-tools '{"queries":["chorus ai list meetings"]}'
```
...
agentlink call-tool '{"toolName":"chorus-ai_list_meetings","input":{...}}'
```

> If Chorus.ai is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
