...
```bash
agentlink search-tools '{"queries":["productboard list features"]}'
```
...
agentlink call-tool '{"toolName":"productboard_list_features","input":{...}}'
```

> If Productboard is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
