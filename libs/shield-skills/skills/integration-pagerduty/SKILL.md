...
```bash
agentlink search-tools '{"queries":["pagerduty list incidents"]}'
```
...
agentlink call-tool '{"toolName":"pagerduty_list_incidents","input":{...}}'
```

> If PagerDuty is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
