...
```bash
agentlink search-tools '{"queries":["aweber list subscribers"]}'
```
...
agentlink call-tool '{"toolName":"aweber_list_subscribers","input":{...}}'
```

> If AWeber is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
