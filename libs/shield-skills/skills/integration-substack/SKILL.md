...
```bash
agentlink search-tools '{"queries":["substack get publication"]}'
```
...
agentlink call-tool '{"toolName":"substack_get_publication","input":{...}}'
```

> If Substack is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
