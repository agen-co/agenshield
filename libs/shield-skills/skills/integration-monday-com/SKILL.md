...
```bash
agentlink search-tools '{"queries":["monday com get boards"]}'
```
...
agentlink call-tool '{"toolName":"monday-com_get_boards","input":{...}}'
```

> If monday.com is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
