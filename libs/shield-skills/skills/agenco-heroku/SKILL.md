...
```bash
agenco search-tools '{"queries":["heroku list apps"]}'
```
...
agenco call-tool '{"toolName":"heroku_list_apps","input":{...}}'
```

> If Heroku is not connected, run `agenco list-connected-integrations` to check, then connect it via the Shield UI.
