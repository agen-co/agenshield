...
```bash
agentlink search-tools '{"queries":["sentry list organizations"]}'
```
...
agentlink call-tool '{"toolName":"sentry_list_organizations","input":{...}}'
```

> If Sentry is not connected, run `agentlink list-connected-integrations` to check, then connect it via the Shield UI.
