...
```bash
agenco search-tools '{"queries":["hubspot marketing list forms"]}'
```
...
agenco call-tool '{"toolName":"hubspot-marketing_list_forms","input":{...}}'
```

> If HubSpot Marketing is not connected, run `agenco list-connected-integrations` to check, then connect it via the Shield UI.
