 ...
 *
 * A unified command proxy installed at {hostHome}/.agenshield/bin/shield-exec.
...
export function shieldExecPath(hostHome?: string): string {
  const home = hostHome || process.env['HOME'] || '';
  return home ? `${home}/.agenshield/bin/shield-exec` : SHIELD_EXEC_PATH;
}
...
/**
 * Generate shield-exec content with the correct shebang for the given hostHome.
 */
