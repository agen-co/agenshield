...
import type { InstallContext, InstallStep, PipelineOptions, PipelineState, PipelineResult, StepResult } from './types.js';
...
export async function runPipeline(
  ...
  for (let i = 0; i < pipeline.length; i++) {
    ...
    // 5. Execute — scope onLog to emit per-step logs
    const originalOnLog = ctx.onLog;
    ctx.onLog = (message: string) => {
      originalOnLog(message);
      ctx.onStepLog?.(step.id, message);
    };
    ...
    try {
      ...
      ctx.onStepLog?.(step.id, `Error: ${message}`);
    ...
    // Restore original onLog after step completes
    ctx.onLog = originalOnLog;
    ...
    // 8. Dynamic step injection (resolve after run + output merge, so it can use run outputs)
    if (step.resolve) {
