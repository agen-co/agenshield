...
case "$1" in
  clone|fetch|push|pull|ls-remote)
    ...
    exec ${config.shieldClientPath} git "$@"
    ;;
  *)
...
if [ -x "${config.nodeBinPath}" ]; then
  ...
  echo "node-bin not found at ${config.nodeBinPath}" >&2
  exit 1
fi
`,
  },
...
exec ${config.shieldClientPath} open "$URL"
`,
  },
...
# Route through broker
exec ${config.shieldClientPath} ssh "$@"
`,
  },
...
# Route through broker
exec ${config.shieldClientPath} scp "$@"
`,
  },
};
...
export async function installGuardedShell(
  ...
  const { GUARDED_SHELL_CONTENT, ZDOT_ZSHRC_CONTENT } = await import('../shell/guarded-shell.js');
  const { GUARDED_SHELL_PATH, ZDOT_DIR, ZDOT_ZSHENV_CONTENT } = await import('../legacy.js');
 ...
 *
 * Writes shield-exec to {hostHome}/.agenshield/bin/shield-exec (root-owned, mode 755),
...
export async function installShieldExec(
  ...
  const { generateShieldExecContent, shieldExecPath, PROXIED_COMMANDS } = await import('../shell/shield-exec.js');
  const socketGroupName = userConfig.groups.socket.name;
  const wrapperConfig = getDefaultWrapperConfig(userConfig);
  const installed: string[] = [];
  const execPath = shieldExecPath();
  const execContent = generateShieldExecContent();

  try {
    // 1. Write shield-exec to shared bin dir (root-owned, 755)
    await execAsync(`sudo mkdir -p "$(dirname "${execPath}")"`);
    await execAsync(`sudo tee "${execPath}" > /dev/null << 'SHIELDEXECEOF'
${execContent}
SHIELDEXECEOF`);
    await execAsync(`sudo chmod 755 "${execPath}"`);
    await execAsync(`sudo chown root:wheel "${execPath}"`);
    ...
    for (const cmd of PROXIED_COMMANDS) {
      ...
      try {
        ...
        await execAsync(`sudo ln -s "${execPath}" "${symlinkPath}"`);
        installed.push(cmd);
      } catch {
...
if [ -x "${wrapperConfig.nodeBinPath}" ]; then
  ...
  echo "node-bin not found at ${wrapperConfig.nodeBinPath}" >&2
  exit 1
fi
`;
 ...
 * "type":"module") to `{hostHome}/.agenshield/lib/interceptor/register.cjs`
...
export async function deployInterceptor(
  ...
  const targetPath = `${baseDir}/lib/interceptor/register.cjs`;
  ...
  try {
    ...
    // Ensure target directory exists
    await execAsync(`sudo mkdir -p "${baseDir}/lib/interceptor"`);
...
/**
 * Copy the broker binary to {hostHome}/.agenshield/bin/
...
export async function copyBrokerBinary(
  ...
  try {
    ...
    // Create a package.json so Node.js treats .js files as ESM.
    ...
    await execAsync(
      `sudo tee "${baseDir}/package.json" > /dev/null << 'PKGJSONEOF'
{"type":"module"}
PKGJSONEOF`
    );
    await execAsync(`sudo chown root:wheel "${baseDir}/package.json"`);
    await execAsync(`sudo chmod 644 "${baseDir}/package.json"`);
 ...
 * When the binary runs from `{hostHome}/.agenshield/bin/`, this resolves to
 * `{hostHome}/.agenshield/lib/libnode.127.dylib` — which doesn't exist unless
 * we copy it there.
 *
...
async function copyNodeDylibs(
  ...
  try {
    ...
    for (const line of lines) {
      ...
      // Copy to {hostHome}/.agenshield/lib/ (directory already exists from setup)
      const resolvedHome = hostHome || process.env['HOME'] || '';
      const libDir = resolvedHome ? `${resolvedHome}/.agenshield/lib` : '/opt/agenshield/lib';
      const targetPath = `${libDir}/${dylibName}`;
      try {
...
export async function copyNodeBinary(
  ...
  const resolvedHostHome = hostHome || process.env['HOME'] || '';
  const baseDir = resolvedHostHome ? `${resolvedHostHome}/.agenshield` : '/opt/agenshield';
  const targetPath = `${baseDir}/bin/node-bin`;
  const socketGroupName = userConfig?.groups?.socket?.name || 'ash_socket';
  ...
  try {
    ...
    const dylibs = await copyNodeDylibs(srcPath, socketGroupName, hostHome);
 ...
 *
 * The installed node binary is then copied to {hostHome}/.agenshield/bin/node-bin
 ...
 * `{hostHome}/.agenshield/bin/node-bin`. This function replaces the NVM binary
...
export async function patchNvmNode(options: {
  ...
  const { nodeBinaryPath, agentUsername, socketGroupName, interceptorPath, socketPath, httpPort, hostHome, verbose, onLog } = options;
  const log = onLog || ((msg: string) => verbose && process.stderr.write(`[SETUP] ${msg}\n`));

  try {
    // 1. Verify the real binary has already been copied to {hostHome}/.agenshield/bin/node-bin
    const resolvedHostHome = hostHome || process.env['HOME'] || '';
    const baseDir = resolvedHostHome ? `${resolvedHostHome}/.agenshield` : '/opt/agenshield';
    const nodeBinTarget = `${baseDir}/bin/node-bin`;
    try {
...
export async function installPresetBinaries(options: {
  ...
  // 1. Install node binary to {hostHome}/.agenshield/bin/node-bin (if 'node' in requiredBins)
  if (requiredBins.includes('node')) {
    ...
    if (options.nvmResult?.success) {
      ...
      if (resolvedNvmResult.success) {
        ...
        log('Copying NVM node binary to shared bin dir');
        ...
        log('Copying node binary to shared bin dir');
  ...
  if (needsInterceptor) {
    log('Deploying interceptor to shared lib dir');
  ...
  if (needsSeatbelt) {
    try {
      ...
      const { generateAgentProfileFromConfig, installSeatbeltProfiles } = await import('../enforcement/seatbelt.js');
