...
const ENFORCEMENT_VARIANT: Record<string, 'error' | 'warning'> = {
  kill: 'error',
  alert: 'warning',
};

interface ProcessPolicyListProps {
...
const ProcessRow = memo(function ProcessRow({
  ...
  const isManaged = policy.tier === 'managed';

  return (
    <PolicyRow onClick={() => !readOnly && onEdit(policy)} sx={readOnly ? { cursor: 'default' } : undefined}>
      <Box onClick={(e) => e.stopPropagation()} sx={{ flexShrink: 0 }}>
        <Switch
          checked={policy.enabled}
          onChange={(e) => onToggle(policy.id, e.target.checked)}
          disabled={busy || isManaged || readOnly}
        />
      </Box>

      <PolicyName>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
          {isManaged && <Lock size={12} style={{ flexShrink: 0, opacity: 0.6 }} />}
      ...
      <PolicyMeta>
        ...
        {policy.enforcement && (
          <StatusBadge
            ...
            variant={ENFORCEMENT_VARIANT[policy.enforcement] ?? 'warning'}
            dot={false}
            size="small"
          />
        )}
  ...
  return a.id === b.id && a.enabled === b.enabled && a.name === b.name &&
    a.action === b.action && a.enforcement === b.enforcement &&
    a.tier === b.tier &&
...
export function ProcessPolicyList({
  policies,
  onToggle,
  onEdit,
  onDelete,
