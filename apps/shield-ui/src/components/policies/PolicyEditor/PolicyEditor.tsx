...
interface PolicyEditorProps {
  ...
  defaultTarget?: 'skill' | 'command' | 'url' | 'filesystem' | 'process' | 'router';
}
...
export function PolicyEditor({ policy, onSave, onCancel, onDirtyChange, onFocusChange, error, hideSecrets, title, saveLabel, saving, defaultTarget }: PolicyEditorProps) {
  ...
  const initial = {
    ...
    target: resolvedTarget as 'skill' | 'command' | 'url' | 'filesystem' | 'process' | 'router',
  ...
  useEffect(() => {
    setFormData({
      name: policy?.name ?? '',
      action: (policy?.action ?? null) as 'allow' | 'deny' | 'approval' | null,
      target: (policy?.target ?? defaultTarget ?? 'command') as 'skill' | 'command' | 'url' | 'filesystem' | 'process' | 'router',
      patterns: policy?.patterns.join('\n') ?? '',
      enabled: policy?.enabled ?? true,
