...
import { CheckCircle, AlertTriangle, Plus, Puzzle } from 'lucide-react';
...
export function SkillEnvSection({ onAddSecret, disabled }: SkillEnvSectionProps) {
  ...
  const missingCount = requirements.filter((r) => !r.fulfilled).length;
    ...
    <Card sx={{ mb: 3, opacity: disabled ? 0.45 : 1, transition: 'opacity 0.2s ease', pointerEvents: disabled ? 'none' : 'auto' }}>
      <CardContent sx={{ p: 0 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, px: 2, pt: 2, pb: 1 }}>
          ...
          {missingCount > 0 && (
            <Badge badgeContent={missingCount} color="warning" sx={{ '& .MuiBadge-badge': { fontSize: 10, height: 16, minWidth: 16 } }} />
          )}
        </Box>
