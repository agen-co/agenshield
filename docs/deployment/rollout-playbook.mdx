...
<img
  src="../images/page-heroes/deployment-rollout-playbook.png"
  alt="Rollout Playbook cover — observe first, enforce when quiet."
  noZoom
/>
...
<Note>
  ...
  should read [Working with your agents](../using/working-with-agents.mdx).
</Note>
...
Decide these in the [Frontegg Portal](https://portal.frontegg.com) first. None
of them touch an endpoint.

| Decision                 | Recommendation for a first rollout                                                                                                 |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------- |
| Default enforcement mode | **`monitor`.** Change this only when you have evidence.                                                                            |
| Which agents to govern   | Start with the one your team actually uses most — Claude Code, Cursor, Codex CLI, Gemini CLI, or any other agent from the catalog. |
...
<Frame caption="A trace — one agent run as a tree of process, file, and network events, each with its decision.">
  <img
    src="../images/portal/trace.png"
    alt="Trace view in the Frontegg Portal showing a single agent run as a nested timeline of process, filesystem, and network events, with an allowed network row highlighted and a callout reading: network egress inside the run."
  />
</Frame>
...
| Endpoint health       | Missing check-in, inactive extension, stale policy   | Repair the endpoint, or re-enroll it               |
