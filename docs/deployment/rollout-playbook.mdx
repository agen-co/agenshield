...
<Note>
  ...
  should read [Working with a protected agent](/using/protected-agents).
</Note>
...
Then create an [**install campaign**](/deployment/campaigns). It produces the
...
Follow the [Quickstart](/getting-started/quickstart) on each, or the
[MDM guide](/deployment/mdm/overview) if the fleet is managed. What
you are proving:
...
[Telemetry](/configuration/telemetry) is where all of that is collected, and
[Enrolled devices](/deployment/devices) tells you whether the fleet is actually
...
[Rules and policy](/configuration/policies) covers how a rule is built from
grants; [Agent resources](/configuration/agent-resources) covers the skills and
...
See [Enforcement modes](/configuration/enforcement-modes) for the exact
...
<Columns cols={2}>
  <Card title="Enforcement modes" icon="shield-check" href="/configuration/enforcement-modes">
    Exact semantics of monitor, audit, and enforce.
  </Card>
  <Card title="MDM deployment" icon="building-2" href="/deployment/mdm/overview">
