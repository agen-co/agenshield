...
icon: laptop
description: Read fleet health in the AgenShield console — what each device state means, what a device detail view tells you, and the difference between revoking a device and offboarding it.
---

Once a campaign starts producing enrollments, **Devices** is where the fleet
lives. Every Mac that completed enrollment appears here, whichever way it was
...
rules, no matter what the console shows centrally.
...
| **Activity**  | Everything this Mac's protected agents did, and the decision on each action                                          |
...
| ------------ | --------------------------------------------------------- | ---------------------------------------- |
| **Revoke**   | Stops receiving policy. Marked quarantined in the console | **Still installed.** Nothing is removed  |
...
<Warning>
  ...
  AgenShield installed and merely unmanaged. Use **Offboard** in the AgenShield
  console. See [Microsoft Intune](/deployment/mdm/intune#offboard-a-device).
</Warning>
...
<Steps>
  <Step title="Enrollments match your MDM group">
    Endpoints in the console should converge on the size of the group you
