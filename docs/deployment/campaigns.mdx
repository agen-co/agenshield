...
icon: megaphone
description: Create an install campaign in the AgenShield console — the enrollment token, the install command, and the ready-made MDM artifacts every rollout starts from.
---
...
<Note>
  A campaign is the first of five things you set up. The whole console journey
...
**Devices → New campaign.** There are only two fields:
...
for you — see [Microsoft Intune](/deployment/mdm/intune).
...
<Frame caption="The campaign detail view — the install command, install URL, and enrollment token, ready to copy.">
  <img
    src="/images/console/campaign-created.png"
    alt="Campaign created panel in the AgenShield console showing a copyable install command, install URL, and enrollment token, with a callout reading: one command enrolls a device."
  />
</Frame>
...
| Artifact              | Use it for                                                                              |
| --------------------- | --------------------------------------------------------------------------------------- |
| **Install command**   | Paste-and-run, one Mac. This is what the [Quickstart](/getting-started/quickstart) uses |
...
you download already contains your campaign token and console URL — there is
nothing to hand-edit:

```text
https://<your-console>/resources/campaigns/v1/<campaign-token>/agenshield.mobileconfig
```
...
[MDM enrollment](/deployment/mdm/overview) for what each payload does, and the
...
- It authorizes **enrollment only**. It cannot read policy, read telemetry, or
  change anything in your console.
...
enroll with. [MDM enrollment](/deployment/mdm/overview) covers how to confirm.
...
[Enrolled devices](/deployment/devices).
...
<Columns cols={2}>
  <Card title="MDM enrollment" icon="building-2" href="/deployment/mdm/overview">
