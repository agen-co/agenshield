...
```bash
# Runs as: root | Timeout: 15s
BROKER_LABEL="com.agenshield.broker.openclaw"
...
<dict>
    ...
    <array>
        <string>/bin/bash</string>
        <string>/Users/ash_openclaw_agent/.agenshield/bin/broker-launcher.sh</string>
    </array>
...
```bash
# Runs as: root | Timeout: 15s
sudo launchctl bootout system/com.agenshield.$PROFILE_BASE.gateway 2>/dev/null; true
sudo launchctl bootstrap system "/Library/LaunchDaemons/com.agenshield.$PROFILE_BASE.gateway.plist"
sudo launchctl kickstart system/com.agenshield.$PROFILE_BASE.gateway
```
